<!--
 * @desc: 产品目录
 * @Date: 2024-10-18 10:38:07
 * @LastEditTime: 2024-10-18 10:38:12
-->
<template>
<div>
    <van-field
...
import { listProdKindTree } from '@/apis/common';
...
const initData = async () => {
  const { data } = await listProdKindTree({});
  if (data && data.length) {
    let ans = data.map((item: any) => ({ ...item, open: false, disabled: props.onlySelectChild ? item.hasChild : false }));
