 ...
 * @LastEditTime: 2024-10-09 19:38:29
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.inclusive.loan.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'inclusive-loan' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.inclusive.loan.kindProdType,
  prodName: '',
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/inclusive/loan/${item.prodId}`);
};
</script>
