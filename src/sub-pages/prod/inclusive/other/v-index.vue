 ...
 * @LastEditTime: 2024-10-09 19:39:03
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.inclusive.other.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'inclusive-other' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.inclusive.other.kindProdType,
  prodName: '',
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/inclusive/other/${item.prodId}`);
};
</script>
