 ...
 * @LastEditTime: 2024-10-09 19:33:46
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.corporation.deposit.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
            ...
            <van-form class="gform" @submit="onSubmit">
              <van-cell-group>
                <van-field
                  ...
                  clearable
                ></van-field>
                <van-field label="存款期限（天）" label-align="top">
                  <template #input>
                    <van-field
                      v-model="reqParams.minInvestDay"
                      input-align="center"
                      type="digit"
                      placeholder="最小存款期限"
                    ></van-field>
                    -
                    <van-field
                      v-model="reqParams.maxInvestDay"
                      input-align="center"
                      type="digit"
                      placeholder="最大存款期限"
                    ></van-field>
...
defineOptions({ name: 'corporation-deposit' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.corporation.deposit.kindProdType,
...
// 提交
const onSubmit = () => {
    if (reqParams.value.maxInvestDay && reqParams.value.minInvestDay) {
    if (Number(reqParams.value.minInvestDay) > Number(reqParams.value.maxInvestDay)) {
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/corporation/deposit/${item.prodId}`);
};
</script>
