 ...
 * @LastEditTime: 2024-10-09 19:37:38
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.corporation.finance.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
        ...
        <van-dropdown-menu ref="dropdownMenuRef">
          <van-dropdown-item>
            ...
            <van-form class="gform" @submit="onSubmit">
              <van-cell-group>
                <dadp-select
                  v-model="reqParams.ridkLevel"
                  label="风险等级"
                  :options="getCombo('riskLevel')"
                  name="riskLevel"
                  input-align="right"
                  placeholder="请选择"
                  clearable
...
defineOptions({ name: 'corporation-finance' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.corporation.finance.kindProdType,
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/corporation/finance/${item.prodId}`);
};
</script>
