 ...
 * @LastEditTime: 2024-10-09 19:26:08
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.corporation.backletter.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
        ...
        <van-dropdown-menu ref="dropdownMenuRef">
          <van-dropdown-item>
            ...
            <van-form class="gform" @submit="onSubmit">
              <van-cell-group>
                <dadp-select
                  v-model="reqParams.currency"
                  label="币种"
                  :options="getCombo('currency')"
                  name="currency"
                  input-align="right"
                  placeholder="请选择"
                  clearable
                ...
                <van-field
                  ...
                  clearable
                ></van-field>
                <van-field label="期限（月）" label-align="top">
                  <template #input>
                    <van-field
                      v-model="reqParams.minInvestMonth"
                      type="digit"
                      name="minInvestMonth"
                      placeholder="请输入最小期限（月）"
                      label-width="8em"
                      input-align="right"
                      clearable
                    ></van-field>
                    -
                    <van-field
                      v-model="reqParams.maxInvestMonth"
                      type="digit"
                      name="minInvestMonth"
                      placeholder="请输入最大期限（月）"
                      label-width="8em"
                      input-align="right"
                      clearable
                    ></van-field>
...
import { listCProd } from '@/apis/prod';
import { getCombo } from '@/hook/code-hook';
import { moduleComp } from '../../hook';

defineOptions({ name: 'corporation-backletter' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.corporation.backletter.kindProdType,
...
// 提交
const onSubmit = () => {
  if (reqParams.value.minInvestMonth && reqParams.value.maxInvestMonth) {
    if (Number(reqParams.value.minInvestMonth) > Number(reqParams.value.maxInvestMonth)) {
      showToast('存款期限最大值不能比最小值小！');
    } else {
      onSearch();
      dropdownMenuRef.value.close();
    }
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/corporation/backletter/${item.prodId}`);
};
</script>
