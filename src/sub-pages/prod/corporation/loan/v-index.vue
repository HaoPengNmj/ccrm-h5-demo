 ...
 * @LastEditTime: 2024-10-09 19:37:35
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.corporation.loan.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'corporation-loan' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.corporation.loan.kindProdType,
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/corporation/loan/${item.prodId}`);
};
</script>
