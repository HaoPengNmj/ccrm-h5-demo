 ...
 * @LastEditTime: 2024-10-09 19:36:18
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.corporation.other.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'corporation-other' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.corporation.other.kindProdType,
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/corporation/other/${item.prodId}`);
};
</script>
