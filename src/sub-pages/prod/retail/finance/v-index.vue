 ...
 * @LastEditTime: 2024-10-09 19:32:16
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.retail.finance.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'retail-finance' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.retail.finance.kindProdType,
  prodName: '',
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/retail/finance/${item.prodId}`);
};
</script>
