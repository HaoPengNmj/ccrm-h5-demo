 ...
 * @LastEditTime: 2024-10-09 19:32:09
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.retail.fund.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'retail-fund' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.retail.fund.kindProdType,
  prodName: '',
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/retail/fund/${item.prodId}`);
};
</script>
