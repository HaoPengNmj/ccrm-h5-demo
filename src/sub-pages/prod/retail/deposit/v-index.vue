...
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.retail.deposit.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'retail-deposit' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.retail.deposit.kindProdType,
  prodName: '',
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/retail/deposit/${item.prodId}`);
};
</script>
