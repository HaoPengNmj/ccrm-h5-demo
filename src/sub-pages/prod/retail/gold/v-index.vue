 ...
 * @LastEditTime: 2024-10-09 19:32:04
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.retail.gold.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'retail-gold' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.retail.gold.kindProdType,
  prodName: '',
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/retail/gold/${item.prodId}`);
};
</script>
