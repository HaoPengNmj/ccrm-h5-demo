 ...
 * @LastEditTime: 2024-10-09 19:31:43
-->
<template>
  <van-sticky>
    <van-tabs v-model:active="reqParams.activeTab" @change="onSearch">
      <van-tab v-for="item in getCombo(moduleComp.retail.loan.codeCombo)" :name="item.value" :title="item.content"></van-tab>
    </van-tabs>
...
defineOptions({ name: 'retail-loan' });
...
// 请求参数
const reqParams = ref({
  kindProdType: moduleComp.retail.loan.kindProdType,
  prodName: '',
...
const handleClick = (item: any) => {
  router.push(`/prod-dtl/retail/loan/${item.prodId}`);
};
</script>
