<template>
  ...
  <dadp-refresh-list
    ref="refreshListRef"
    class="refresh-list"
    :requestApi="listMyPotentialCust"
    :reqParams="{
      ...reqParams,
    }"
  >
    <template v-slot="{ data }">
      <dadp-card avatar="task">
        <template #title>
          <van-space>
            <span>{{ data.custName }}({{ data.potentialCustId }})</span>
          </van-space>
        </template>
        <template #subTitle>
          <van-space>
  ...
  <van-action-bar>
    <van-action-bar-button type="primary" text="新增潜客" @click="router.push('/pcm-epq/add')" />
  </van-action-bar>
</template>
...
import { listMyPotentialCust } from '@/apis/cust';
import { potentailCustMark } from '@/apis/map';
...
const setMark = async (item) => {
  const { code, msg } = await potentailCustMark({
    custId: item.potentialCustId,
